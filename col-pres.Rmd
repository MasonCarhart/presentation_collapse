...
sflights_dt = as.data.table(flights)[key(year)]
```
...
microbenchmark(storms_lubridate_col = storms_dt %>% 
                  ftransform(date = make_date(year, month, day)) %>%  
                  fselect(date, wind, pressure),
                      storms_lubridate_dply = storms_dt %>% 
                  dplyr::mutate(date = make_date(year, month, day)) %>%  
                  dplyr::select(date, wind, pressure))
...
microbenchmark(collapse = sflights_dt %>% fgroup_by(year) %>% 
                    get_vars(16) %>% fmean ,
               data.table = sflights_dt[, lapply(.SD, mean), 
                    keyby = year, .SDcols = 16], times = 100)
...
microbenchmark(collapse = sflights_dt %>% fgroup_by(year) %>% 
                    get_vars(16) %>% fmean,
               data.table = sflights_dt[, mean := mean(distance, na.rm=T),keyby = year], times = 100)
